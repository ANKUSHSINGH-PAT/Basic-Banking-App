...
import com.example.basic_banking_app.Data.User;
import com.example.basic_banking_app.ListAdapter.SendToUserAdapter;
import com.example.basic_banking_app.ListAdapter.UserListAdapter;
import com.example.basic_banking_app.R;
import com.example.basic_banking_app.DB.Transaction_Contact.TransactionEntry;
import com.example.basic_banking_app.DB.User_Contact.UserEntry;
...
public class SendToUser extends AppCompatActivity implements SendToUserAdapter.OnUserListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (bundle != null) {
            fromUserAccountName = bundle.getString("FROM USER NAME");
            fromUserAccountNo = bundle.getInt("FROM ACCOUNT NO");
        ...
        // Show list of items
        recyclerView = findViewById(R.id.send_to_user);
    ...
    private void calculateAmount() {
        //Toast.makeText(this, "Paisa cut", Toast.LENGTH_LONG).show();
        Integer currentAmount = Integer.parseInt(fromUserAccountBalance);
        Log.d("Balance","C_Bal="+fromUserAccountBalance);
        Integer transferAmountInt = Integer.parseInt(transferAmount);
        Log.d("Balance","T_Bal="+transferAmount);
        ...
        Log.d("Balance","Remaining amount"+fromUserAccountNo);
        Log.d("Balance","Remaining amount"+toUserAccountNo);
        new UserHelper(this).updateAmount1(fromUserAccountNo, remainingAmount);
        new UserHelper(this).updateAmount(toUserAccountNo, increasedAmount);
       // Log.d("Dta","transferAmountInt"+increasedAmount);
       // Log.d("Balance","Remaining amount"+remainingAmount);
    }
